...
function deterministicFeedback(
  ...
  return `Thanks for working through these hospitality-analytics cases.${overall?.value ? ` You scored ${overall.value}.` : ""} Strong answers here come from reasoning about the situation — looking at multiple numbers together, weighing how reliable the evidence is, and resisting single-number or 'obvious' explanations.`;
}
...
async function generateSubjectVariant(
  ...
  const system =
    "You are an assessment author writing ORIGINAL restaurant & hospitality analytics questions for an intro course. " +
  ...
  const user =
    `Write ${count} new, distinct restaurant & hospitality analytics case questions within the scope above.\n` +
    `For style only (do NOT copy these): ${JSON.stringify(examplePrompts)}.`;
  const out = await chatJson<{
