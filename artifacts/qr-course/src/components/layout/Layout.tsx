      ...
      <div className="p-6 border-b border-border">
        <Link href="/dashboard">
          <div className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="font-serif font-semibold text-lg tracking-tight">Basic Revenue Management & Pricing Analytics</span>
          </div>
        </Link>
      </div>
  ...
  return (
    <div className="sticky top-0 z-10 flex items-center justify-end gap-2 px-6 py-3 border-b border-border bg-background/80 backdrop-blur">
      ...
      {user && (
        ...
        >
          {user.email ?? user.displayName ?? user.username ?? "Account"}
        </span>
      )}
      <button
        onClick={logout}
